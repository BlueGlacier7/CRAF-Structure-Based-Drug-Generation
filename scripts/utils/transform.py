...
class AtomComposer(object):
    ...
    def run(self, data):
        ...
        data.idx_ligand_ctx_in_cpx = torch.arange(num_ligand_ctx_atom, dtype=torch.long) 
        data.idx_protein_in_cpx = torch.arange(num_protein_atom, dtype=torch.long) + num_ligand_ctx_atom
...
def collate_fn(data_list, collate_keys=COLLATE_KEYS):
    ...
    for i, d in enumerate(data_list):
        if hasattr(d, 'atom_label') and isinstance(d.atom_label, torch.Tensor):
            if d.atom_label.numel() == 0 or d.atom_label.dim() != 1:
                print(f"Skip sample {i}: atom_label shape {getattr(d, 'atom_label', None).shape}")
                continue
        else:
            print(f"Skip sample {i}: atom_label missing or not tensor")
            continue
