import torch
from scripts.utils import Protein, Ligand, ComplexData, torchify_dict, is_in_ring
import numpy as np
...
for ix, sdf_supplier in enumerate(tqdm(sdf_list)):
    mol_list = list(Chem.ForwardSDMolSupplier(sdf_supplier, removeHs=True))
...
np.save(os.path.splitext(processed_path)[0] + '_Keys.npy', index_list)
