...
config = EasyDict(
    {'deq_coeff':0.9, 'hidden_channels':64, 'hidden_channels_vec':16, 'bottleneck':[8,2], 'use_conv1d':False,
...
class CRAF_main(nn.Module):
    def __init__(self, config) -> None:
        super(CRAF_main, self).__init__()
        self.config = config
    ...
    def get_loss(self, data):
        ...
        p_y = self.pos_predictor.get_mdn_probability(abs_mu, sigma, pi, data.y_pos)
        if (p_y < 0).any():
            warnings.warn("Warning: Negative probability detected in loss_pos calculation!")
            print("p_y=", p_y)
        loss_pos = -torch.log(
        ...
        out_dict = {
            'loss':loss, 'loss_atom':ll_atom, 'loss_edge':ll_edge, 
